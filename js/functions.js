		...
		switch (eleccionServicio) {
			case 1:
				if (agua <= saldoCuenta) {
					saldoCuenta -= agua;
					alert(
						`Has pagado el servicio Agua. Se te descontaron $${agua},\ntu saldo anterior era $${saldoCuenta +
							agua} tu saldo actual es $${saldoCuenta}`
					);
			...
			case 2:
				if (luz <= saldoCuenta) {
					saldoCuenta -= luz;
					alert(
						`Has pagado el servicio Luz. Se te descontaron $${luz},\ntu saldo anterior era $${saldoCuenta +
							luz} tu saldo actual es $${saldoCuenta}`
					);
					actualizarSaldoEnPantalla();
				} else {
					alert(`No hay suficiente saldo en tu cuenta para pagar este servicio.`);
			...
			case 3:
				if (telefono <= saldoCuenta) {
					...
					alert(
						`Has pagado el servicio Telefono. Se te descontaron $${telefono},\ntu saldo anterior era $${saldoCuenta +
							telefono} tu saldo actual es $${saldoCuenta}`
					);
					actualizarSaldoEnPantalla();
				} else {
					alert(`No hay suficiente saldo en tu cuenta para pagar este servicio.`);
			...
			case 4:
				if (internet <= saldoCuenta) {
					...
					alert(
						`Has pagado el servicio Internet. Se te descontaron $${internet},\ntu saldo anterior era $${saldoCuenta +
							internet} tu saldo actual es $${saldoCuenta}`
					);
					actualizarSaldoEnPantalla();
				} else {
					alert(`No hay suficiente saldo en tu cuenta para pagar este servicio.`);
...
const funcionParaTransferencia = () => {
	if (password === passwordIngresado) {
		...
		if (transferencia <= saldoCuenta) {
			let ingresoCuentaTransferUsuario = prompt(
				`Por favor ingresa el numero de cuenta para realizar la transferencia`
			);
			ingresoCuentaTransferUsuario = parseInt(ingresoCuentaTransferUsuario);
			if (
				ingresoCuentaTransferUsuario === cuentasAmigas.cuentaUno ||
				ingresoCuentaTransferUsuario === cuentasAmigas.cuentaDos
			) {
				...
				alert(
					`Solo puedes transferir dinero a cuentas amigas, la cuenta nro. ${ingresoCuentaTransferUsuario}, no es una cuenta amiga.`
				);
			}
		} else {
...
const functionInicioSesion = () => {
	passwordIngresado = parseInt(prompt('Por favor ingresa tu password(8981)'));
...
const funcionParaExtraccion = () => {
	if (nuevaExtraccion <= saldoCuenta && nuevaExtraccion <= limiteExtraccion && nuevaExtraccion % 100 === 0) {
		...
		actualizarSaldoEnPantalla();
		alert(
			`Has extraido : $${nuevaExtraccion}, tu saldo es de: $${saldoCuenta}, tu saldo era : $${saldoCuenta +
				nuevaExtraccion}`
		);
